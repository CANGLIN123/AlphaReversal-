...
class BacktestEngine:
    ...
    def __init__(self, config: dict = None):
        ...
        self.config = {
            ...
            'score_weights': {
                'j_value': 0.12,               # J值因子
                'volume_pattern': 0.20,        # 量价因子
                'bbi_trend': 0.18,             # BBI趋势
                'reversal': 0.14,              # 反转确认
                'hot_industry': 0.13,          # 行业热度
                'ma60_proximity': 0.13,        # MA60贴近度（新）
                'quality': 0.10,               # 质量因子
            },
    ...
    def run(self, stock_data: Dict[str, pd.DataFrame],
        ...
        for i, date in enumerate(trading_dates):
            ...
            if not stock_rows:
                ...
                # ---- 硬性条件过滤 (向量化) ----
                # 规则1: BBI 必须高于 MA60（BBI在MA60以下的股票直接不做）
                # 规则2: 收盘价必须在 MA60 上方
                # 规则3: J值必须低于阈值（超卖信号）
                # 规则4: 量价形态必须有效
                mask = (
                    ...
                    (df_scores['kdj_j'] < cfg['j_threshold']) &
                    (df_scores['vol_pattern_valid'] == 1) &
                    (df_scores['bbi'] > df_scores['ma60']) &       # ← BBI > MA60 硬性规则
                    (df_scores['close'] > df_scores['ma60'])
                )
                ...
                if len(df_valid) == 0:
                    ...
                    # ---- 因子得分 (向量化，一次算完整列) ----
                    # J值得分：J越低越好（超卖）
                    df_valid['score_j'] = np.maximum(
                        0, (cfg['j_threshold'] - df_valid['kdj_j']) / cfg['j_threshold'] * 100
                    )
                    # 量价得分：直接使用预计算分数
                    df_valid['score_vol'] = np.minimum(100, df_valid['vol_pattern_score'])
                    # BBI 偏离 MA60 得分：偏离越大越好
                    df_valid['score_bbi'] = np.minimum(
                        100, (df_valid['bbi'] - df_valid['ma60']) / df_valid['ma60'] * 500
                    )
                    # 反转确认得分（替代旧动量因子）
                    # 旧动量(ret_5d)对超卖股失效：J<20=股价在跌→ret_5d为负→得分=0
                    # 新因子检查"反转是否已开始"：
                    #   ① J值拐头向上(40分)：今天J > 昨天J，超卖在修复
                    #   ② 站上MA5(35分)：收盘价>5日均线，短线企稳
                    #   ③ 继续缩量(25分)：量比<0.8，抛压持续衰竭
                    j_turning = (
                    ...
                    df_valid['score_reversal'] = np.clip(
                        j_turning * 40 + above_ma5 * 35 + still_shrinking * 25, 0, 100
                    )
                    # 行业热度得分：真正的行业维度计算
                    # ① 计算每个行业近20日平均涨幅
                    # ② 涨幅排名前30%的行业 = 热门行业
                    # ③ 热门行业股票得分=100，其余按排名线性递减
                    if stock_sectors and len(stock_sectors) > 0:
                        ...
                        # 行业涨幅排名 → 分位数映射到[30,100]
                        sector_rank = sector_ret.rank(pct=True)
                        sector_score = 30 + sector_rank * 70  # [30,100]
                        df_valid['score_hot'] = df_valid['sector'].map(sector_score).fillna(50)
                    else:
                        # 降级：无行业数据时用个股涨幅分位数
                        ret20 = df_valid['ret_20d'].fillna(0)
                        df_valid['score_hot'] = np.clip(
                            ret20.rank(pct=True) * 100, 0, 100
                        )
                    # MA60贴近度得分：价格越靠近60日均线，得分越高
                    # 逻辑：贴近均线 = 回调到位，远离均线 = 可能追高
                    # close/MA60 偏离度 → 偏离<2%=100分, 偏离>20%=0分
                    ma60_deviation = np.abs(df_valid['close'] - df_valid['ma60']) / df_valid['ma60']
                    df_valid['score_ma60'] = np.clip(100 - ma60_deviation * 500, 0, 100)
                    # 质量得分：默认中性 50，有基本面数据时向量化叠加
                    df_valid['score_quality'] = 50.0

                    # 基本面 PE/ROE 质量调整（向量化 merge）
                        ...
                        if not funda.empty and 'ts_code' in funda.columns:
                            ...
                            if not funda_sub.empty:
                                ...
                                pe_adj = np.where(
                                    ...
                                    np.clip((20 - df_valid['pe']) / 20 * 25, -25, 25),
                                    0
                                )
                                roe_adj = np.where(
                                    df_valid['roe'].notna() & (df_valid['roe'] > 0),
                                    np.clip((df_valid['roe'] - 5) / 15 * 25, -25, 25),
                                    0
                                )
                    ...
                    w = cfg['score_weights']
                    df_valid['score'] = (
                        df_valid['score_j'] * w['j_value'] +
                        df_valid['score_vol'] * w['volume_pattern'] +
                        df_valid['score_bbi'] * w['bbi_trend'] +
                        df_valid['score_reversal'] * w['reversal'] +
                        df_valid['score_hot'] * w['hot_industry'] +
                        df_valid['score_ma60'] * w['ma60_proximity'] +
                        df_valid['score_quality'] * w['quality']
                    )

                    # 最低得分过滤 + 排名
                    df_valid = df_valid[df_valid['score'] >= cfg['min_score']]
                    df_valid = df_valid.sort_values('score', ascending=False)

                    # 转为与买入循环兼容的 list-of-dict 格式（字段名与改动前完全一致）
                    scores = []
                ...
                if available_slots > 0:
                    ...
                    for s in scores:
                        ...
                        shares = calc_position_size(
                            cash / (available_slots + len(positions) + 1),
                            risk_per_trade, atr, exec_price
                        )
                        ...
                        if total_cost > cash * 0.25:
                            # 单只不超过可用资金的 25%
                            buy_amount = cash * 0.25 - commission - slippage
                        ...
                        if total_cost <= cash:
                            ...
                            if cfg.get('batch_entry_enabled', False) and cfg.get('batch_entry_days', 1) > 1:
                                # 第一批：只买入目标仓位的 batch_entry_ratios[0]
                                batch1_ratio = cfg['batch_entry_ratios'][0]
                                batch1_shares = int(shares * batch1_ratio / 100) * 100
                                if batch1_shares < 100:
                                    batch1_shares = shares  # 太少就不分批了
                                batch1_amount = batch1_shares * exec_price
                                batch1_commission = max(cfg['min_commission'], batch1_amount * cfg['commission'])
                                batch1_slippage = batch1_amount * cfg['slippage']
                                batch1_total = batch1_amount + batch1_commission + batch1_slippage
                                ...
                                if batch1_total <= cash:
                                    ...
                                    positions[code] = {
                                        ...
                                        'cost': exec_price,
                                        'buy_date': exec_date,
                                        'target_shares': shares,          # 目标总股数
                                        'entry_phase': 'building',        # 建仓中
                                        'batch_num': 1,                   # 已完成的批次
                                        'signal_atr': atr,                # 记录信号时的 ATR
                                    }
                                    available_slots -= 1
                            else:
                                # 一次性建仓
    ...
    def _check_sell_signals(self, row, pos: dict, current_price: float,
        ...
        # ================================================================
        # Layer 4: 动态时间止损 — 赚钱多给时间，亏钱按时砍
        # ================================================================
        # 4a. 第7天亏损 → 砍
        if days_held >= 7 and floating_pnl < 0:
        ...
        # 4c. 第15天：亏损砍，盈利续到20天
        if days_held >= 15 and floating_pnl < 0:
            return True, f'第{days_held}天亏损({floating_pnl:.1%})', 1.0

        # 4d. 第20天：亏损砍，盈利续到25天
        if days_held >= 20 and floating_pnl < 0:
            return True, f'第{days_held}天亏损({floating_pnl:.1%})', 1.0

        # 4e. 第25天：终极上限全清
        if days_held >= 25:
            return True, f'持仓超25天({floating_pnl:.1%})', 1.0
    ...
    def _process_batch_entries(self, positions: dict, stock_data: dict,
        ...
        for code, pos in list(positions.items()):
            ...
            # 计算本批次股数
            remaining_ratio = sum(cfg.get('batch_entry_ratios', [0.5, 0.5])[batch_num:])
            if remaining_ratio <= 0:
                pos['entry_phase'] = 'complete'
                continue

            this_ratio = cfg['batch_entry_ratios'][batch_num] / remaining_ratio
