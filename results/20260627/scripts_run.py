...
def main():
    ...
    config = {
        ...
        'trailing_activation': 0.02,        # 浮盈 2% 激活移动止盈
        'partial_profit_taking': True,      # 分批止盈
        'partial_profit_levels': [0.10, 0.20],
        'partial_exit_ratios': [0.3, 0.3],

        # —— 持仓时间 ——
        'max_hold_days': 15,                # 最大持仓天数
        'profit_deadline_days': 7,          # 利润检查日
        'profit_deadline_threshold': 0.0,   # 到期最低收益率
        'bbi_profit_protect_pct': 0.08,     # BBI 保护激活浮盈
        'bbi_profit_exit_ratio': 0.7,       # BBI 保护卖出比例
    ...
    daily_stats = engine.run(valid_stocks, index_df)
