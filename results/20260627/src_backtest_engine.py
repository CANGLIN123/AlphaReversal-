...
class BacktestEngine:
    ...
    def __init__(self, config: dict = None):
        ...
        self.config = {
            ...
            'atr_stop_multiplier': 2.5,        # ATR 硬止损倍数（2→2.5，避免频繁震出）
            ...
            'trailing_activation': 0.02,       # 浮盈 2% 激活移动止盈（3→2，更快锁利）
        ...
        for i, date in enumerate(trading_dates):
            ...
            if not stock_rows:
                ...
                if len(df_valid) == 0:
                    ...
                    # ④ 放量异动得分：前20天有放量阳线+量随价升（参考买点共性 #2）
                    df_valid['score_surge'] = df_valid['had_volume_surge'].fillna(0) * 100
                    ...
                    w = cfg['score_weights']
                    df_valid['score'] = (
                        df_valid['score_oversold'] * w.get('oversold', 0.18) +
                        df_valid['score_shrink'] * w.get('volume_shrink', 0.16) +
                        df_valid['score_small_candle'] * w.get('small_candle', 0.14) +
                        df_valid['score_surge'] * w.get('prior_surge', 0.12) +
                        df_valid['score_reversal'] * w.get('reversal', 0.12) +
                        df_valid['score_bbi'] * w.get('bbi_trend', 0.10) +
                        df_valid['score_ma60'] * w.get('ma60_proximity', 0.07) +
                        df_valid['score_hot'] * w.get('hot_industry', 0.05) +
                        df_valid['score_amihud'] * w.get('amihud', 0.03) +
                        df_valid['score_quality'] * w.get('quality', 0.03)
                    )
                    ...
                    for s in scores:
                        ...
                        if code in stock_data and date in stock_data[code].index:
                            ...
                            if t_limit_up or t_limit_down:
                                continue  # T日涨跌停，无法以合理价格成交

                            # T+1日涨跌停检查
                            if exec_date in stock_df.index:
                                t1_idx = stock_df.index.get_loc(exec_date)
                                t1_limit_up, t1_limit_down = is_price_limit_day(stock_df, t1_idx)
                                if t1_limit_up:
                                    continue  # T+1涨停，买不到
                                if t1_limit_down:
                                    continue  # T+1跌停，不在跌停板接飞刀

                        # ==== 开盘跳空保护：T+1开盘价 vs T日收盘价 ====
                        signal_price = s['price']  # T日收盘价
                        if exec_price > signal_price * 1.035:
                            # T+1开盘跳空>3.5%，说明信号被市场抢跑，追高风险大
                            continue
                        # 开盘大幅低开>5%，可能是突发利空，跳过
    ...
    def _check_sell_signals(self, row, pos: dict, current_price: float,
        ...
        # ================================================================
        # 4a. 第12天亏损 → 砍
        if days_held >= 12 and floating_pnl < 0:
            return True, f'第{days_held}天亏损({floating_pnl:.1%})', 1.0

        # 4b. 第13天微利 → 砍（13-18天是死亡区间，微利大概率变亏损）
        if days_held >= 13 and floating_pnl < 0.02:
