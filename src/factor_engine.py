...
def _add_rebound_elasticity(df: pd.DataFrame) -> pd.DataFrame:
    """
    反弹弹性因子 — 预测超卖后反弹幅度的核心因子

    四个维度：
      1. 跌幅深度 (40%): 从60日最高点回撤了多少 → 跌得越深弹性越大
      2. 下跌缩量比 (30%): 下跌期均量 / 上涨期均量 → 缩量下跌=散户卖，弹性好
      3. 爆量阳线强度 (20%): 那根放量阳线的力度 → 主力进场越猛弹性越大
      4. 缩量持续天数 (10%): 连续量比<0.8的天数 → 蓄力越久弹性越足

    设计逻辑：
      缩量下跌 = 散户恐慌抛售、主力未动 → 反弹又猛又快（好的超卖）
      放量下跌 = 主力也在出货 → 反弹无力甚至继续跌（差的超卖）
    """
    n = len(df)
    close = df['close'].values
    volume = df['volume'].values

    rebound = np.full(n, 50.0)  # 默认中性50分

    for i in range(60, n):
        # — ① 跌幅深度：60日最高点回撤 —
        high_60d = np.max(df['high'].values[i - 59:i + 1])
        drawdown = (close[i] - high_60d) / high_60d  # 负数，越负跌越深
        dd_score = np.clip(abs(drawdown) * 500, 0, 100)  # -20%=100分, -10%=50分

        # — ② 下跌缩量比：下跌日均量 / 上涨日均量 —
        rets = df['close'].pct_change().values[i - 59:i + 1]
        vols = volume[i - 59:i + 1]
        down_days = rets < 0
        up_days = rets > 0
        down_vol = np.mean(vols[down_days]) if down_days.sum() > 0 else np.mean(vols)
        up_vol = np.mean(vols[up_days]) if up_days.sum() > 0 else np.mean(vols)
        if up_vol > 0:
            vol_ratio_direction = down_vol / up_vol
            # 缩量下跌 < 1.0 = 好（散户卖主力不动），放量下跌 > 1.0 = 坏
            vol_score = np.clip((1.5 - vol_ratio_direction) / 1.0 * 100, 0, 100)
        else:
            vol_score = 50
        ...
        for j in range(i - 24, i + 1):  # 前25天
            if j > 0 and vol_ma5[j] > 0 and volume[j] > vol_ma5[j] * 1.5:
                ret_day = (close[j] - close[j - 1]) / close[j - 1]
                if ret_day > 0.05:
                    # 强度 = 涨幅倍率 × 量倍率
                    strength = min(ret_day / 0.05, 2.0) * min(volume[j] / (vol_ma5[j] * 1.5), 2.0)
                    surge_strength = max(surge_strength, strength)
        surge_score = np.clip(surge_strength / 4.0 * 100, 0, 100)

        # — ④ 缩量持续天数：连续量比<0.8 —
        consecutive = 0
        for j in range(i, max(i - 30, -1), -1):
            if j > 0 and 'volume_ma20' in df.columns:
                v20 = df['volume_ma20'].values[j]
                if v20 > 0 and volume[j] / v20 < 0.8:
                    consecutive += 1
                else:
                    break
        shrink_score = np.clip(consecutive / 10 * 100, 0, 100)

        # 综合弹性分
        rebound[i] = (dd_score * 0.40 + vol_score * 0.30
                      + surge_score * 0.20 + shrink_score * 0.10)

    df['rebound_elasticity'] = rebound
    return df
...
def compute_all_factors(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # —— 放量异动检测（参考买点共性 #2: 量随价升）——
    # 两种模式满足其一即可：
    #   A. 前25天内有单根爆量阳线（量>5日均量×1.5 且 涨幅>5%）— 主力突击建仓
    #   B. 前20天内有连续堆量阳线（≥5根阳线且每根量>5日均量）— 温和吸筹
    n_rows = len(df)
    vol_ma5_local = df['volume'].rolling(5).mean().values
    daily_ret_local = df['close'].pct_change().values
    is_yang = (df['close'].values > df['open'].values)  # 阳线

    surge_big = np.zeros(n_rows, dtype=bool)     # 模式A：单根爆量
    surge_stack = np.zeros(n_rows, dtype=bool)   # 模式B：连续堆量

    for ii in range(5, n_rows):
        # 模式A：单日爆量阳线（涨幅>5% + 量>5日均量×1.5）
        if (daily_ret_local[ii] > 0.05
                and vol_ma5_local[ii] > 0
                and df['volume'].values[ii] > vol_ma5_local[ii] * 1.5):
            surge_big[ii] = True

        # 模式B：连续堆量阳线（阳线 + 量>5日均量）
        if is_yang[ii] and vol_ma5_local[ii] > 0 and df['volume'].values[ii] > vol_ma5_local[ii]:
            surge_stack[ii] = True

    # 模式A：前25天窗口
    had_big_surge = pd.Series(surge_big).rolling(25).max().fillna(0).values
    # 模式B：前20天内≥5根堆量阳线
    had_stack_surge = (pd.Series(surge_stack).rolling(20).sum().fillna(0).values >= 5).astype(float)

    df['had_volume_surge'] = ((had_big_surge + had_stack_surge) >= 1).astype(int)
    df['surge_type_big'] = had_big_surge.astype(int)       # 调试用：是否爆量模式
    df['surge_type_stack'] = had_stack_surge.astype(int)   # 调试用：是否堆量模式

    # —— 反弹弹性因子（预测反弹幅度）——
    df = _add_rebound_elasticity(df)
